...
只读中文界面包含流程总览（`/`）、本地种子目录（`/torrents`）和种子详情（`/torrents/$hash`）。首页展示当前运行的有限事件窗口；目录持久查询已保存的 v1 metadata，并显示后台索引回填进度。路径只作安全文本展示，不提供按需联网下载、原始 bencode、piece hash 或 torrent 导出。后端接口和字段以[观测专题](../docs/domains/monitoring.md)为准。
