...
[security](../../src/dht/security.rs)实现 IPv4／IPv6 CRC32C Node ID 生成与校验。节点区分 compliant、compatible、local_exempt；本地豁免仍须通过 AddressPolicy。不合规公网节点可被服务，其返回值仅作候选，不计安全近邻成功；当前没有使用远端 token 的出站 announce 客户端。

响应与错误包含顶层 compact ip。收到观察值前须完成 transaction 与来源匹配，普通响应另须通过身份及结构校验；非法可选 ip 被忽略，不丢弃原本有效响应。

显式外部地址优先。自动观察按地址族独立保存最多 64 票，TTL 10 分钟；IPv4 /24、IPv6 /48 各前缀一票。至少三个不同前缀同意且占有效票数不少于三分之二才确认。首次确认可切换，之后至少间隔 30 分钟；未确认时保留缓存或随机兼容身份，不阻塞引导。

dispatcher 串行结算旧查询及采样预约，持久化后更换路由位置并重新验证联系人，保留 socket、handle 和共享预算。旧 transaction 不复用，受影响采集按 IdentityChanged 本地延期，不累加远端失败。采样输出通道和冷却继续由同一所有者持有；存储故障交给 Session 收尾。
