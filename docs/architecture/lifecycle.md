...
app 在启动阶段绑定 loopback，显式启用时绑定失败返回启动错误。Session 拥有 Monitor 及其刷新任务、HTTP 连接集合；目录回填由 Session 独立持有，在 fetch 或 monitor 启用时启动。回填停止令牌阻止新批次，已进入数据库线程的单条操作仍由数据库关闭屏障确认。监控任务提前退出只停止监控，数据库线程故障仍走原故障路径。
