...
后端启用监控后，可使用独立[前端](../../web/README.md)查看当前状态和有限历史。开发服务器和静态反向代理仅监听 loopback，浏览器始终请求同源 `/api`。生产构建使用[代理示例](../../web/nginx.conf)，远端查看通过 SSH 转发前端端口。冻结显示只影响当前网页，采集继续运行；网页不提供任务控制或 metadata 下载。
