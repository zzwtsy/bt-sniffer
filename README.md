...
日志通过 4,096 条有界异步队列写入 stderr，`RUST_LOG` 默认 `warn,bt_sniffer=info`。新增 `--log-format {text,json}`，默认 text；文件/服务器推荐 json。文本颜色仅对终端启用，支持 `NO_COLOR`。配置、字段迁移和 systemd 示例见[日志说明](docs/logging.md)。正常收到 Ctrl-C 或 Unix SIGTERM 时会停止采集、保存最终路由快照、排空待保存结果并关闭数据库；清理共用 30 秒期限，失败返回非零退出码。运行时退出后最多再等待系统 DNS 线程 1 秒，这不代表底层 DNS 调用可以被取消。
