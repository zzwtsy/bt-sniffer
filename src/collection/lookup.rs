...
//!
//! LookupPacer 仅控制查询节奏；TCP 许可由 worker 在 tcp_limits 中申请。
use crate::collection::diagnostics::metrics;
...
use crate::dht::dispatcher::DiscoveredNode;
use crate::dht::dispatcher::GetPeersResponse;
use crate::dht::dispatcher::QueryError;
use crate::dht::dispatcher::RemoteNode;
use crate::dht::dispatcher::RpcProgress;
...
async fn family(
    ...
    progress: Arc<RpcProgress>,
    fault: Arc<Mutex<Option<QueryError>>>,
    ...
    let result = search(
        ...
        output.peers.clone(),
        Some(output),
        move |node| query(handle.clone(), node, hash, pacer.clone(), progress.clone()),
    )
    .await;
...
where
    ...
    F: std::future::Future<Output = Result<GetPeersResponse, QueryError>>,
{
                ...
                if collected.len() < 32 && !collected.contains(&peer) {
                    if let Some(output) = &output {
                        if collected.is_empty() {
                            output.metrics.add(Counter::LookupFirstPeer, 1);
                            output
                                .metrics
                                .observe(metrics::Timing::FirstPeer, output.start.elapsed());
                        }
...
impl Drop for LookupReport {
    fn drop(&mut self) {
        self.metrics.add(Counter::LookupsFinished, 1);
        self.metrics
            .add(Counter::RpcSent, self.progress.sent.load(Ordering::Relaxed));
        if !self.peers.lock().expect("peer 结果锁").is_empty() {
            self.metrics.add(Counter::LookupWithPeers, 1);
        }
    }
}
...
pub(super) async fn stream(
    ...
    let mut families: FuturesUnordered<_> = handles
        .iter()
        .map(|h| {
            let (pacer, found_seeds) = (pacer.clone(), found_seeds.clone());
            let progress = progress.clone();
            let fault = fault.clone();
            let output = output.clone();
            async move {
                family(h.clone(), hash, pacer, found_seeds, progress, fault, output).await;
            }
        })
        .collect();
    let work = async {
...
mod tests {
    ...
    async fn rate_budget_is_shared_across_destinations_and_per_ip() {
        let pacer = LookupPacer::default();
        let first = "127.0.0.1".parse().unwrap();
        let second = "127.0.0.2".parse().unwrap();
        let start = Instant::now();
        pacer.pace(first).await;
        pacer.pace(second).await;
        assert_eq!(start.elapsed(), Duration::from_millis(100));
        pacer.pace(first).await;
        assert_eq!(start.elapsed(), Duration::from_secs(1));
        assert!(pacer.rate.lock().await.ips.len() <= 2);
    }
}
