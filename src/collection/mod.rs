...
impl Collector {
    ...
    async fn log_status(
        ...
        stats.log(false);
        let recent_active = self.store.recent_active_jobs(self.now()?).await?;
        let first_attempt_waiting = self
            .store
            .first_attempt_waiting(self.now()?, self.config.policy)
            .await?;
        let buffer_limit = self
            .config
            .max_active
            .min(self.config.concurrency.saturating_mul(4));
        self.store.first_attempt_backlog(self.now()?).await?.log();
