...
use crate::{
    ...
    krpc::{
        CompactNodesV4, CompactNodesV6, CompactPeerAddress, InfoHashSamples, KrpcMessage,
        MessageType, NodeId, QueryArgs, QueryMethod, ResponseArgs, Token,
    },
...
async fn await_metadata(store: &StorageHandle) {
...
async fn sample_or_history(family: AddressFamily, source: DiscoverySource) {
    ...
    if source == DiscoverySource::Sampling {
        session
            .start_sampling(
                0,
...
async fn verified() -> VerifiedMetadata {
    ...
    .unwrap()
    .fetch(hash(), &[peer], &CancellationToken::new())
    .await
    .unwrap();
    task.await.unwrap();
    metadata
        ...
        loop {
            ...
            if request.message.q == Some(QueryMethod::GetPeers) {
                ...
                reply.r.as_mut().unwrap().nodes =
                    Some(CompactNodesV4(vec![crate::krpc::CompactNodeV4 {
                        id: b_id,
                        address,
                    }]));
            }
...
async fn lookup_promotes_reserve_on_both_families() {
    for family in [AddressFamily::Ipv4, AddressFamily::Ipv6] {
                            ...
                            args.nodes = (family == AddressFamily::Ipv4).then(|| {
                                CompactNodesV4(
                                    contacts
                                        .iter()
                                        .skip(1)
                                        .map(|(id, a)| crate::krpc::CompactNodeV4 {
                                            id: *id,
                                            address: match a {
                                                SocketAddr::V4(a) => *a,
                                                _ => unreachable!(),
                            ...
                            args.nodes6 = (family == AddressFamily::Ipv6).then(|| {
                                CompactNodesV6(
                                    contacts
                                        .iter()
                                        .skip(1)
                                        .map(|(id, a)| crate::krpc::CompactNodeV6 {
                                            id: *id,
                                            address: match a {
                                                SocketAddr::V6(a) => *a,
                                                _ => unreachable!(),
        ...
        let result = lookup::lookup(
            ...
            target,
            Arc::new(lookup::Network::default()),
        )
        .await;
...
async fn local_tcp_wait_timeout_does_not_consume_failure_attempt() {
    ...
    let outcome = run_job(
        ...
        vec![],
        MetadataFetcher::new(MetadataConfig::default()).unwrap(),
        network.clone(),
        AddressPolicy::LocalUnicast,
...
async fn fixed_pipeline_scenario(family: AddressFamily, jobs: usize) -> serde_json::Value {
    use crate::dht::{dispatcher::DhtDispatcher, routing::RoutingTable, traffic::Budget};
    let start = tokio::time::Instant::now();
    ...
    dispatcher.budget = budget.clone();
            ...
            args.nodes = (family == AddressFamily::Ipv4).then(|| {
                CompactNodesV4(
                    addresses
                        .iter()
                        .enumerate()
                        .map(|(n, a)| crate::krpc::CompactNodeV4 {
            ...
            args.nodes6 = (family == AddressFamily::Ipv6).then(|| {
                CompactNodesV6(
                    addresses
                        .iter()
                        .enumerate()
                        .map(|(n, a)| crate::krpc::CompactNodeV6 {
                            id: NodeId([n as u8 + 20; 20]),
                            address: match a {
                                SocketAddr::V6(a) => *a,
                                _ => unreachable!(),
