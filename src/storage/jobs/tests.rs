...
async fn dedup_capacity_recovery_and_stale_generation() {
    ...
    assert_eq!(
        store
            .retry_job(first, 201, RetryReason::Failed("old_error"))
            .await
            .unwrap(),
        UpdateResult::Stale
    );
...
async fn hints_are_bounded_expire_and_dormancy_needs_new_observation() {
    ...
    assert!(s.claim_job(90_000_000).await.unwrap().is_some());
    storage.shutdown().await.unwrap();
}
...
async fn preferred_claims_rotate_borrow_and_respect_hint_policy_and_due_time() {
    ...
    let policy = crate::net::address::AddressPolicy::PublicOnly;
    for id in 1..=12 {
    ...
    for turn in 0..12 {
        let (job, fresh, _) = store
            .claim_preferred(100, Some(turn % 4 < 3), policy)
            .await
            .unwrap()
            .unwrap();
        order.push(job.hash.0[0]);
        assert_eq!(fresh, turn % 4 < 3);
        store
    ...
    assert!(
        store
            .claim_preferred(101, Some(true), policy)
            .await
            .unwrap()
            .is_none()
    );
    let (_, fresh, _) = store
        .claim_preferred(2_000_000, Some(true), policy)
        .await
        .unwrap()
        .unwrap();
    assert!(!fresh, "全部提示过期后借用普通类");
    storage.shutdown().await.unwrap();
}
...
async fn capacity_still_refreshes_accepted_hints_and_local_deferral_keeps_attempts() {
    ...
    let (job, fresh, _) = s
        .claim_preferred(
            200,
            Some(true),
            crate::net::address::AddressPolicy::PublicOnly,
        )
        .await
        .unwrap()
        .unwrap();
